...
export default function Index() {
  ...
  const applyPromo = () => {
    ...
    if (PROMO_CODES[code]) {
      ...
      setPromoMsg(`Промокод применён! Скидка ${PROMO_CODES[code]}%`);
    } else {
  ...
  const openWhatsApp = (name: string, price: number) => {
    const finalPrice = promoApplied ? Math.round(price * (1 - promoApplied / 100)) : price;
    const text = encodeURIComponent(`Хочу купить: ${name} за ${finalPrice}₽`);
    window.open(`https://wa.me/79011505959?text=${text}`, "_blank");
  };
  ...
  return (
    <div className="min-h-screen" style={{ background: "linear-gradient(135deg, #080808 0%, #150303 50%, #0a0606 100%)" }}>
      
      {/* Ambient blobs */}
      <div className="fixed inset-0 overflow-hidden pointer-events-none z-0">
        <div style={{ position: "absolute", width: 500, height: 500, borderRadius: "50%", background: "radial-gradient(circle, #c0392b18 0%, transparent 70%)", top: "5%", left: "-5%" }} />
        <div style={{ position: "absolute", width: 400, height: 400, borderRadius: "50%", background: "radial-gradient(circle, #d4a01714 0%, transparent 70%)", bottom: "15%", right: "5%" }} />
        <div style={{ position: "absolute", width: 250, height: 250, borderRadius: "50%", background: "radial-gradient(circle, #c0392b0d 0%, transparent 70%)", top: "55%", left: "35%" }} />
      </div>

      {/* ── HEADER ── */}
      <header className="relative z-10 sticky top-0" style={{ background: "rgba(8,8,8,0.94)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(212,160,23,0.18)" }}>
        <div className="max-w-6xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <div className="shrink-0">
            <h1 className="rgb-logo text-2xl md:text-3xl">ReallyTime</h1>
            <p className="text-xs hidden sm:block" style={{ color: "#d4a017", opacity: 0.6, fontFamily: "Oswald, sans-serif", letterSpacing: "0.15em" }}>GAME STORE</p>
          </div>
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <button onClick={() => setCartOpen(true)}
              ...
              style={{ background: "rgba(192,57,43,0.12)", border: "1px solid rgba(192,57,43,0.35)", color: "#ff7b7b" }}>
              <Icon name="ShoppingCart" size={13} />
              <span className="hidden sm:inline">Корзина</span>
              {cartCount > 0 && (
                <span className="absolute -top-1 -right-1 w-4 h-4 rounded-full text-xs flex items-center justify-center font-bold"
                  style={{ background: "#c0392b", color: "white", fontSize: "10px" }}>
                  {cartCount}
                </span>
              )}
            </button>
        ...
        <div className="md:hidden flex overflow-x-auto gap-1 px-4 pb-2 scrollbar-hide">
          {(["donates", "cases-donate", "cases-titles", "promo"] as const).map(t => (
            <button key={t} onClick={() => setTab(t)}
              className={`shrink-0 px-3 py-1.5 rounded-md text-xs font-semibold transition-all ${tab === t ? "tab-active" : "text-gray-500"}`}>
              {t === "donates" ? "Донаты" : t === "cases-donate" ? "Кейсы Донат" : t === "cases-titles" ? "Кейсы Титулы" : "Промокоды"}
            </button>
          ))}
        </div>
      ...
      <main className="relative z-10 max-w-6xl mx-auto px-4 py-8">
        ...
        <div className="text-center mb-8 animate-fade-in-up">
          <p className="text-xs tracking-widest text-gray-600" style={{ fontFamily: "Oswald, sans-serif" }}>
            ИГРОВОЙ МАГАЗИН · БЫСТРАЯ ДОСТАВКА
          </p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {DONATES.map((d, i) => (
                <div key={d.id}
                  ...
                  <div className="mb-4">
                    <h3 className="text-xl font-bold text-white mb-1" style={{ fontFamily: "Oswald, sans-serif", letterSpacing: "0.05em" }}>{d.name}</h3>
                    <div className="text-2xl font-black" style={{ color: "#d4a017", fontFamily: "Oswald, sans-serif" }}>{d.price}₽</div>
                  </div>
                  <div className="flex gap-2">
                    <button onClick={() => addToCart(`donate-${d.id}`, d.name, d.price)}
                      className="flex-1 py-2 rounded-lg text-sm font-bold transition-opacity hover:opacity-80"
                      style={{ background: "rgba(212,160,23,0.12)", border: "1px solid rgba(212,160,23,0.35)", color: "#d4a017" }}>
                      + В корзину
                    </button>
                    <button onClick={() => setBuyModal({ name: d.name, price: d.price })}
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {CASES_DONATE.map((c, i) => (
                <div key={c.id}
                  className="shop-card relative rounded-xl p-5 bg-gradient-to-br from-red-950 to-zinc-900 border border-red-800 animate-fade-in-up"
                  style={{ animationDelay: `${i * 0.06}s`, animationFillMode: "both" }}>
                  ...
                  <div className="flex gap-2">
                    <button onClick={() => addToCart(`case-donate-${c.id}`, `Кейс Донат ×${c.qty}`, c.price)}
                      className="flex-1 py-2 rounded-lg text-sm font-bold"
                      style={{ background: "rgba(212,160,23,0.12)", border: "1px solid rgba(212,160,23,0.35)", color: "#d4a017" }}>
                      + В корзину
                    </button>
                    <button onClick={() => setBuyModal({ name: `Кейс Донат ×${c.qty}`, price: c.price })}
                      className="flex-1 py-2 rounded-lg text-sm font-bold"
                      style={{ background: "linear-gradient(135deg, #c0392b, #a93226)", color: "white" }}>
                      Купить
                    </button>
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {CASES_TITLES.map((c, i) => (
                <div key={c.id}
                  className="shop-card relative rounded-xl p-5 bg-gradient-to-br from-yellow-950 to-zinc-900 border border-yellow-800 animate-fade-in-up"
                  style={{ animationDelay: `${i * 0.06}s`, animationFillMode: "both" }}>
                  <div className="text-5xl mb-3 text-center">👑</div>
                  <h3 className="text-center font-bold text-white mb-1" style={{ fontFamily: "Oswald, sans-serif" }}>
                    {c.qty} {c.qty === 1 ? "кейс" : c.qty < 5 ? "кейса" : "кейсов"}
                  ...
                  <div className="text-center text-2xl font-black mb-4" style={{ color: "#d4a017", fontFamily: "Oswald, sans-serif" }}>{c.price}₽</div>
                  <div className="flex gap-2">
                    <button onClick={() => addToCart(`case-title-${c.id}`, `Кейс Титулы ×${c.qty}`, c.price)}
                      className="flex-1 py-2 rounded-lg text-sm font-bold"
                      style={{ background: "rgba(212,160,23,0.12)", border: "1px solid rgba(212,160,23,0.35)", color: "#d4a017" }}>
                      + В корзину
                    </button>
                    <button onClick={() => setBuyModal({ name: `Кейс Титулы ×${c.qty}`, price: c.price })}
                      className="flex-1 py-2 rounded-lg text-sm font-bold"
                      style={{ background: "linear-gradient(135deg, #c0392b, #a93226)", color: "white" }}>
                      Купить
                    </button>
        ...
        {tab === "promo" && (
          <div className="max-w-lg mx-auto animate-fade-in-up">
            ...
            <div className="rounded-xl p-6" style={{ background: "rgba(255,255,255,0.03)", border: "1px solid rgba(212,160,23,0.18)" }}>
              ...
              <div className="flex gap-2 mb-4">
                <input type="text" value={promoCode} onChange={e => setPromoCode(e.target.value)}
                  ...
                  className="flex-1 px-4 py-3 rounded-lg text-white font-semibold uppercase"
              ...
              {promoMsg && (
                <div className="px-4 py-3 rounded-lg text-sm font-semibold mb-4"
                  style={{ background: promoApplied ? "rgba(212,160,23,0.12)" : "rgba(192,57,43,0.12)", border: `1px solid ${promoApplied ? "rgba(212,160,23,0.35)" : "rgba(192,57,43,0.35)"}`, color: promoApplied ? "#d4a017" : "#ff7b7b" }}>
                  {promoMsg}
                </div>
              )}
              {promoApplied && (
              ...
              <div className="mt-4 p-3 rounded-lg text-center" style={{ background: "rgba(192,57,43,0.06)", border: "1px solid rgba(192,57,43,0.15)" }}>
      ...
      {cartOpen && (
        <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setCartOpen(false)}>
          <div className="w-full max-w-md rounded-2xl p-6 animate-fade-in-up"
            style={{ background: "#0f0f0f", border: "1px solid rgba(212,160,23,0.25)", maxHeight: "80vh", overflowY: "auto" }}
            ...
            {cart.length === 0 ? (
              ...
              <>
                  ...
                  {cart.map(item => (
                    <div key={item.id} className="flex items-center justify-between p-3 rounded-lg"
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <button onClick={() => removeFromCart(item.id)} style={{ color: "#c0392b" }}>
                ...
                <button
                  onClick={() => { setCartOpen(false); if (cart.length > 0) { const names = cart.map(i => `${i.name} ×${i.qty}`).join(", "); setBuyModal({ name: names, price: discountedTotal }); }}}
      ...
      {buyModal && (
        <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setBuyModal(null)}>
          <div className="w-full max-w-sm rounded-2xl p-6 text-center animate-fade-in-up"
            style={{ background: "#0f0f0f", border: "1px solid rgba(37,211,102,0.25)" }}
            onClick={e => e.stopPropagation()}>
            <div className="text-5xl mb-3">💬</div>
            <h2 className="text-xl font-black text-white mb-2" style={{ fontFamily: "Oswald, sans-serif" }}>ОФОРМЛЕНИЕ</h2>
            <p className="text-gray-500 text-sm mb-2 leading-tight">{buyModal.name}</p>
            ...
            {promoApplied && <p className="text-xs mb-4" style={{ color: "#d4a017" }}>Скидка {promoApplied}% применена</p>}
            <p className="text-xs text-gray-600 mb-5 mt-2">Нажми кнопку — откроется WhatsApp с менеджером</p>
            ...
            <button onClick={() => setBuyModal(null)} className="w-full py-2 rounded-xl text-sm text-gray-600">Отмена</button>
          </div>
        </div>
      )}
      ...
      {/* ── TOPUP MODAL ── */}
      {topupOpen && (
        <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4" onClick={() => setTopupOpen(false)}>
          <div className="w-full max-w-sm rounded-2xl p-6 animate-fade-in-up"
            style={{ background: "#0f0f0f", border: "1px solid rgba(212,160,23,0.25)" }}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-xl font-black" style={{ fontFamily: "Oswald, sans-serif", color: "#d4a017" }}>ПОПОЛНЕНИЕ</h2>
              <button onClick={() => setTopupOpen(false)} style={{ color: "#666" }}><Icon name="X" size={20} /></button>
            </div>
            <p className="text-gray-500 text-sm mb-4">Текущий баланс: <span style={{ color: "#d4a017" }}>{balance}₽</span></p>
      ...
      {adminOpen && (
        <div className="modal-backdrop fixed inset-0 z-50 flex items-center justify-center p-4"
          onClick={() => { setAdminOpen(false); setAdminAuthed(false); setAdminPass(""); setAdminMsg(""); }}>
          <div className="w-full max-w-sm rounded-2xl p-6 animate-fade-in-up"
            style={{ background: "#0d0d0d", border: "1px solid rgba(192,57,43,0.35)" }}
            onClick={e => e.stopPropagation()}>
            <div className="flex items-center justify-between mb-5">
              ...
              <button onClick={() => { setAdminOpen(false); setAdminAuthed(false); setAdminPass(""); setAdminMsg(""); }} style={{ color: "#555" }}>
                <Icon name="X" size={20} />
              </button>
            </div>
            ...
            {!adminAuthed ? (
              ...
              <>
                <div className="p-3 rounded-lg mb-4" style={{ background: "rgba(212,160,23,0.08)", border: "1px solid rgba(212,160,23,0.18)" }}>
                  <p className="text-xs text-gray-500 mb-1">Текущий баланс</p>
                  <p className="text-2xl font-black" style={{ color: "#d4a017", fontFamily: "Oswald, sans-serif" }}>{balance}₽</p>
                </div>
      ...
      <footer className="relative z-10 text-center py-8 mt-8">
        <div className="section-divider w-56 mx-auto mb-4" />
        <p className="text-gray-700 text-xs" style={{ fontFamily: "Oswald, sans-serif" }}>© 2024 REALLYTIME · GAME STORE</p>
        <p className="text-gray-700 text-xs mt-1">Покупки: WhatsApp +7 901 150-59-59</p>
      </footer>
    </div>
  );
}
